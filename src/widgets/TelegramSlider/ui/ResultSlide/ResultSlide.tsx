...
import type { IInternalProgress } from '../../model/types/TelegramSlider.types';
...
type ResultSlideProps = {
	className?: string;
	theme: ResultSlideTheme;
	data: IInternalProgress[] | string[];
};

const ResultSlide = ({ className, theme, data }: ResultSlideProps) => {
	const content: Record<ResultSlideTheme, () => ReactNode> = {
		single: () => (
				...
				{(data as string[]).map((item) => (
					<FlexH as={'li'} gap={'8'} align={'start'} key={item}>
						<span className={styles.result__icon}>
							<Icon icon={<CheckIcon />} size={IconSize.SIZE_14} />
						</span>
						<Text
							text={item.toUpperCase()}
							fontFamily={'montserrat'}
							size={TextSize.S}
							color={TextColor.DARK_SECONDARY}
						/>
					</FlexH>
				))}
			</FlexV>
		...
		double: () => (
			<FlexH as={'ul'} align={'stretch'} justify={'stretch'} gap={'16'}>
				{(data as IInternalProgress[]).map(({ title, progress }) => (
